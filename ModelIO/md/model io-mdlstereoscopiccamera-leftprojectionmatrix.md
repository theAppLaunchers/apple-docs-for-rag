...
Model I/O automatically generates this matrix from the same properties that produce the projectionMatrix property of a monocular camera, additionally using the interPupillaryDistance and leftVergence properties to account for the offset and angle of a stereoscopic camera’s left viewpoint.

A renderer uses this matrix, along with the leftViewMatrix property and model matrices derived from the camera’s position and orientation (the transform property) and the content to be rendered, to transform vertex data to the renderer’s 2D screen space at render time.
