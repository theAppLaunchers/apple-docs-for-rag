...
This function is rarely needed and is likely to be deprecated. Apple recommends using the SKIndexFlush(_:) function along with the default memory size limit for index updates. Refer to the `SKIndexSetMaximumBytesBeforeFlush(_:_:)` function for more information.
