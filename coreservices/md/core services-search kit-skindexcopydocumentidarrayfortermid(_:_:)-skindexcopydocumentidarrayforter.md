...
`SKIndexCopyDocumentIDArrayForTermID` searches a single index for documents that contain a given term. The search uses a term ID, not a term string. To get the ID of a term, use SKIndexGetTermIDForTermString(_:_:).
