...
The document ID identifies a document URL object (of type `SKDocument`) in an index. The ID is available as soon as you add a document URL object to an index using SKIndexAddDocumentWithText(_:_:_:_:) or SKIndexAddDocument(_:_:_:_:).
