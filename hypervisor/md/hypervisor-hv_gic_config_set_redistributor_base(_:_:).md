...
``` source
func hv_gic_config_set_redistributor_base(
    _ config: hv_gic_config_t,
    _ redistributor_base_address: hv_ipa_t
) -> hv_return_t
```
...
Guest physical address for redistributor base aligned to byte value returned by hv_gic_get_redistributor_base_alignment(_:). The redistributor region contains redistributors for all vCPUs that the virtual machine supports.
