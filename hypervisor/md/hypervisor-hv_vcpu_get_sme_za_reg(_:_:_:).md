...
This methods returns an error if `PSTATE.ZA` is `0` (for example, when. za_storage_enabled is `false`), or if the provided value storage isn’t in the range of \[maximum SVL bytes x maximum SVL bytes\].
