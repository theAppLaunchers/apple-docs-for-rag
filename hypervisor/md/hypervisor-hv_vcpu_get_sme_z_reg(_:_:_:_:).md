...
The method returns an error if not in streaming SVE mode, for example ifstreaming_sve_mode_enabled is `false`, or if the value `length` isn’t maximum SVL bytes.
