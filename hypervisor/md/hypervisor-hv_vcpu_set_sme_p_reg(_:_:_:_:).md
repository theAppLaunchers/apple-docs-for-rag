...
``` source
func hv_vcpu_set_sme_p_reg(
    _ vcpu: hv_vcpu_t,
    _ reg: hv_sme_p_reg_t,
...
This method returns an error if `PSTATE.ZA` is `0` (For example, if.za_storage_enabled is `false`), or if the provided value storage isn’t \[maximum SVL bytes x maximum SVL bytes\].
