...
The length of the provided ZA register value, in bytes.
...
This method returns an error if `PSTATE.ZA` is `0` (for example, if za_storage_enabled is `false`), or if the value `length` isn’t the range of \[maximum of `SVL` bytes \* maximum `SVL` bytes\].
