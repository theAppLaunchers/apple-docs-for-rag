...
The number of bytes of memory to use per row of the bitmap. If the `data` parameter is `nil`, passing a value of `0` causes the value to be calculated automatically.
...
The custom release function to call when it is time to release the bitmap data. For the syntax of this function, see the description of the CGBitmapContextReleaseDataCallback data type. You may specify `nil` for this parameter.
