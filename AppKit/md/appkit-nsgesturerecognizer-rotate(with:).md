...
An object encapsulating information about the rotate event.
...
A gesture recognizer monitors events that occur in its view (and any subviews) but does not take part in the responder chain itself. The gesture recognizer receives events before any views do. Use the delaysRotationEvents property to control whether `event` is propagated to the view.
