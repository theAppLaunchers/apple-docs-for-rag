...
An expression of type `T`, where `T` conforms to Numeric.

`expression2`  

A second expression of type `T`, where `T` conforms to Numeric.

`accuracy`  

An expression of type `T`, where `T` conforms to Numeric. This parameter describes the maximum difference between `expression1` and `expression2` for these values to be considered not equal.
