...
- XCTest
- XCTestCase
-  expectation(that:on:options:willSatisfy:) Deprecated
...
func expectation(
    ...
    willSatisfy predicate: XCTKeyPathExpectation.Predicate? = nil
) -> XCTKeyPathExpectation where T : NSObject
```

Deprecated

T and V must be Sendable for use in an asynchronous filtering function. Use expectation(that:on:options:willSatisfy:) to provide an asynchronous filtering function and expectation(that:on:options:willSatisfy:) for a syncronous filtering function instead.
