...
Place IDs are exempt from the storage restrictions in Section 2.5 of Attachment 6 to the Apple Developer Program License Agreement.
...
Use Place IDs to display annotations with MKMapItemAnnotation from MapKit for AppKit and UIKit or mapkit.PlaceAnnotation from MapKit JS. You can also use Place IDs to show more detailed selection accessories, such as popups that automatically display a place’s name, address, hours, and more, with MKSelectionAccessory from MapKit for AppKit and UIKit and mapkit.PlaceSelectionAccessory from MapKit JS.
