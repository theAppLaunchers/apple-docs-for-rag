...
- Added support for automatic reload payments, recurring payment requests, and multiple payment tokens in ApplePayModifier and ApplePayPaymentRequest. See automaticReloadPaymentRequest, recurringPaymentRequest and multiTokenContexts for more information for `Payment Request API`. See automaticReloadPaymentRequest, recurringPaymentRequest, and multiTokenContexts for more information for Apple Pay JS API.

- Added support to ApplePayPaymentTiming by including an automaticReload value that specifies that the payment occurs automatically.

- Added support for merchants to provide order details of a purchase that’s viewable in Wallet using ApplePayPaymentCompleteDetails.

- Added `bancomat` and `bancontact` to supported networks. For more information, see supportedNetworks.
