...
- Added new method openPaymentSetup, which displays the Set up Apple Pay button if the user has not completed setup, or the Apple Pay button otherwise.

- Added CSS support for rendering Apple Pay buttons using the properties `-webkit-appearance: -apple-pay-button`, `-apple-pay-button-type`, and `-apple-pay-button-style`. See Displaying Apple Pay Buttons Using CSS for more information.

- Localized Apple Pay buttons. See Localizing Apple Pay Buttons Using CSS for more information.

- Added `jcb` to supported networks. See supportedNetworks for more information.
