...
The unique identifier for the budget order. You set the `boID` when creating a budget order through the Apple Search Ads UI.
...
This call retrieves a specific assigned budget order for your organization. It also returns completed and canceled orders. Budget orders also return when you use the Create a Campaign or Update a Campaign endpoints.

You can’t set budget order invoicing through the API. You can only fetch a budget order using Get a Budget Order or Get All Budget Orders.
