...
If you want to specify a ShapeStyle like a HierarchicalShapeStyle or a Material as the background, use background(_:ignoresSafeAreaEdges:) instead. To specify a Shape or InsettableShape, use background(_:in:fillStyle:). To configure the background of a presentation, like a sheet, use presentationBackground(alignment:content:).
