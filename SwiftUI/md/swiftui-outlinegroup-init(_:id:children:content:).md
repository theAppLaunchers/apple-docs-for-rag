...
) where Data == Binding, C : MutableCollection, C : RandomAccessCollection, E == C.Element
```

Available when `Data` conforms to `RandomAccessCollection`, `ID` conforms to `Hashable`, `Parent` conforms to `View`, `Parent` is `Leaf`, and `Subgroup` is `DisclosureGroup`.
