...
iOS 12.0+iPadOS 12.0+macOS 10.12+tvOS 12.0+Xcode 13.2+
...
A *fragment* is a possible change to the render targets. The rasterizer determines which pixels of the render target are covered by the primitive. Only fragments whose pixel centers are inside the triangle are rendered.
...
Now that the functions are complete, you can create a render pipeline that uses them. First, get the default library and obtain an MTLFunction object for each function.
...
Next, create an MTLRenderPipelineState object. Render pipelines have more stages to configure, so you use an MTLRenderPipelineDescriptor to configure the pipeline.
...
Often, you use buffers (MTLBuffer) to pass data to shaders. However, when you need to pass only a small amount of data to the vertex function, as is the case here, copy the data directly into the command buffer.
...
Run the sample again. The render pipeline uses the color value from the first vertex (called the *provoking vertex*) uniformly across the triangle, and it ignores the colors from the other two vertices. You can use a mix of flat shaded and interpolated values, simply by adding or omitting the `flat` qualifier on your vertex function’s outputs. The Metal Shading Language specification defines other attribute qualifiers you can also use to modify the rasterization behavior.
