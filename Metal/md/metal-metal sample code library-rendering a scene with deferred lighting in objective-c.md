...
iOS 12.0+iPadOS 12.0+macOS 10.13+tvOS 12.0+Xcode 12.5+
...
```
id  renderEncoder = [commandBuffer renderCommandEncoderWithDescriptor:_viewRenderPassDescriptor];
...
```
id renderEncoder = [commandBuffer renderCommandEncoderWithDescriptor:_GBufferRenderPassDescriptor];
...
```
id renderEncoder = [commandBuffer renderCommandEncoderWithDescriptor:_finalRenderPassDescriptor];
...
deferred_directional_lighting_fragment_traditional(
    ...
    texture2d          albedo_specular_GBuffer [[ texture(AAPLRenderTargetAlbedo) ]],
    texture2d          normal_shadow_GBuffer   [[ texture(AAPLRenderTargetNormal) ]],
    texture2d         depth_GBuffer           [[ texture(AAPLRenderTargetDepth)  ]])
```
