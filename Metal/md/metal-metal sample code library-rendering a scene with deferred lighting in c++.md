...
iOS 13.0+iPadOS 13.0+macOS 11.0+tvOS 13.0+Xcode 13.2+
...
deferred_directional_lighting_fragment_traditional(
    ...
    texture2d      albedo_specular_GBuffer [[ texture(RenderTargetAlbedo) ]],
    texture2d      normal_shadow_GBuffer   [[ texture(RenderTargetNormal) ]],
    texture2d     depth_GBuffer           [[ texture(RenderTargetDepth)  ]])
```
...
const std::vector& vertexBuffers = m_icosahedronMesh.vertexBuffers();
pRenderEncoder->setVertexBuffer( vertexBuffers[0].buffer(), vertexBuffers[0].offset(), BufferIndexMeshPositions );

const std::vector& icosahedronSubmesh = m_icosahedronMesh.submeshes();
