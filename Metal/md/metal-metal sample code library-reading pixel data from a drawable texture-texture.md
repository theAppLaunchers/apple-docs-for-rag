...
```
id readTexture = view.currentDrawable.texture;
...
const id pixelBuffer = [self readPixelsWithCommandBuffer:commandBuffer
...
Next, the method creates a MTLBlitCommandEncoder, which provides commands that copy data between Metal resources, fill resources with data, and perform other similar resource-related tasks that don’t directly involve computation or rendering. The sample encodes a blit command to copy the texture data to the beginning of the new buffer. It then ends the blit pass.
