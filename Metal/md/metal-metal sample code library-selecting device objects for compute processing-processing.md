...
A MTLBuffer can’t be directly transferred between different devices; its data must be transferred via system memory.

### Allocate System Memory for a Buffer

The sample calls the `vm_allocate` function to allocate a page-aligned buffer, `updateAddress`, backed by system memory. The sample then calls the makeBuffer(bytesNoCopy:length:options:deallocator:) method to create a new MTLBuffer, `_updateBuffer`, backed by the same system memory used for the previous buffer.
