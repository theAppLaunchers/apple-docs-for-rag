

- WebKit
-  DOMNamedNodeMap Deprecated

Class

# DOMNamedNodeMap

macOS 10.4–10.14Deprecated

``` source
class DOMNamedNodeMap
```

## Topics

### Instance Properties

var length: UInt32

### Instance Methods

func getNamedItem(String!) -> DOMNode!

func getNamedItemNS(String!, localName: String!) -> DOMNode!

func item(UInt32) -> DOMNode!

func removeNamedItem(String!) -> DOMNode!

func removeNamedItemNS(String!, localName: String!) -> DOMNode!

func setNamedItem(DOMNode!) -> DOMNode!

func setNamedItemNS(DOMNode!) -> DOMNode!

## Relationships

### Inherits From

- DOMObject

### Conforms To

- CVarArg
- CustomDebugStringConvertible
- CustomStringConvertible
- Equatable
- Hashable
- NSCopying
- NSObjectProtocol

## See Also

### Document Object Model (DOM) APIs

class DOMAbstractViewDeprecated

class DOMAttrDeprecated

class DOMBlobDeprecated

class DOMCDATASectionDeprecated

class DOMCharacterDataDeprecated

class DOMCommentDeprecated

class DOMCounterDeprecated

class DOMCSSCharsetRuleDeprecated

class DOMCSSFontFaceRuleDeprecated

class DOMCSSImportRuleDeprecated

class DOMCSSMediaRuleDeprecated

class DOMCSSPageRuleDeprecated

class DOMCSSPrimitiveValueDeprecated

class DOMCSSRuleDeprecated

class DOMCSSRuleListDeprecated

