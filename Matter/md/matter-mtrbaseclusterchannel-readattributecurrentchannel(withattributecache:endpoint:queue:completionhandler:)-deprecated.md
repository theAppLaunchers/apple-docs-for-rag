...
-  readAttributeCurrentChannel(withAttributeCache:endpoint:queue:completionHandler:) Deprecated
...
``` source
class func readAttributeCurrentChannel(
    withAttributeCache attributeCacheContainer: MTRAttributeCacheContainer,
    endpoint: NSNumber,
    queue: dispatch_queue_t,
    completionHandler: @escaping (MTRChannelClusterChannelInfo?, (any Error)?) -> Void
)
```
...
class func readAttributeCurrentChannel(
    ...
    endpoint: NSNumber,
    queue: dispatch_queue_t
) async throws -> MTRChannelClusterChannelInfo
```
