

- Matter
-  MTRElectricalMeasurementClusterGetMeasurementProfileCommandParams Deprecated

Class

# MTRElectricalMeasurementClusterGetMeasurementProfileCommandParams

iOS 16.1–18.2DeprecatediPadOS 16.1–18.2DeprecatedMac Catalyst 16.1–18.2DeprecatedmacOS 13.0–15.2DeprecatedtvOS 16.1–18.2DeprecatedvisionOS 1.0–2.2DeprecatedwatchOS 9.1–11.2Deprecated

``` source
class MTRElectricalMeasurementClusterGetMeasurementProfileCommandParams
```

Deprecated

This command has been removed

## Topics

### Instance Properties

var attributeId: NSNumber

var numberOfIntervals: NSNumber

var serverSideProcessingTimeout: NSNumber?

var startTime: NSNumber

var timedInvokeTimeoutMs: NSNumber?

## Relationships

### Inherits From

- NSObject

### Conforms To

- CVarArg
- CustomDebugStringConvertible
- CustomStringConvertible
- Equatable
- Hashable
- NSCopying
- NSObjectProtocol

