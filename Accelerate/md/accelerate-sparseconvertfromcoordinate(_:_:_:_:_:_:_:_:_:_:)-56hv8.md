...
The workspace of size `rowCount * sizeof(int)`.

## Return Value

A new SparseMatrix_Double object, using the memory you provide in the storage parameter. After you finish using a sparse matrix, call SparseCleanup(_:) to release its references to any memory that the Sparse Solvers library allocates.
