...
The 16-bit-per-channel ARGB value that the function writes to the destination based on the `copyMask` value.
...
This fused operation provides the same functionality as vImagePermuteChannels_ARGB16U(_:_:_:_:) followed by `vImageOverwriteChannelsWithScalar_ARGB16U` but offers higher performance than calling the two functions separately.
