...
The 32-bit-per-channel ARGB value that the function writes to the destination based on the `copyMask` value.
...
This fused operation provides the same functionality as vImagePermuteChannels_ARGBFFFF(_:_:_:_:) followed by vImageOverwriteChannelsWithScalar_ARGBFFFF(_:_:_:_:_:) but offers higher performance than calling the two functions separately.
