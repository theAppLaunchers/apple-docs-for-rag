...
Combine’s `Publisher/tryMap(_:)` operator performs a function similar to that of map(_:) in the Swift standard library: it uses a closure to transform each element it receives from the upstream publisher. You use `Publisher/tryMap(_:)` to transform from one kind of element to another, and to terminate publishing when the map’s closure throws an error.

The following example uses an array of numbers as the source for a collection based publisher. A `Publisher/tryMap(_:)` operator consumes each integer from the publisher and uses a dictionary to transform it from its Arabic numeral to a Roman equivalent, as a String. If the `Publisher/tryMap(_:)`’s closure fails to look up a Roman numeral, it throws an error. The `Publisher/tryMap(_:)` operator catches this error and terminates publishing, sending a `Subscribers/Completion/failure(_:)` that wraps the error.
