...
An optional source for the event, such as an entity or a scene. Set to `nil` to listen for all events of the event type within the RealityViewCameraContent.

`componentType`  

An optional component type to filter events to if the event is of the type ComponentEvents. Set to `nil` to listen for all events of the event type within the RealityViewCameraContent.
