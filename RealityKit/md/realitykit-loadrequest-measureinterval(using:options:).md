...
In the example below, a 1-second Timer is used as the data source for an event publisher; the `Publisher/measureInterval(using:options:)` operator reports the elapsed time between the reception of events on the main run loop:
