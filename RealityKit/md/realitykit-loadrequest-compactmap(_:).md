...
Combine’s `Publisher/compactMap(_:)` operator performs a function similar to that of compactMap(_:) in the Swift standard library: the `Publisher/compactMap(_:)` operator in Combine removes `nil` elements in a publisher’s stream and republishes non-`nil` elements to the downstream subscriber.
