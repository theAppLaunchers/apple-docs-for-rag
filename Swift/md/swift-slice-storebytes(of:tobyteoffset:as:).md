...
The memory written to must not extend beyond the memory region represented by the buffer pointer slice—that is, `offset + MemoryLayout.size` must be less than or equal to the slice’s `count`.
