...
The type to which this memory will be bound.

`value`  

The value used to initialize this memory.
...
After calling this method on a raw pointer `p`, the region starting at `self` and continuing up to `p + MemoryLayout.stride` is bound to type `T` and initialized. If `T` is a nontrivial type, you must eventually deinitialize the memory in this region to avoid memory leaks.
