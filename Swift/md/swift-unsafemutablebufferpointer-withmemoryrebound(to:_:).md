...
func withMemoryRebound(
    to type: T.Type,
    _ body: (UnsafeMutableBufferPointer) throws(E) -> Result
...
The type to temporarily bind the memory referenced by this buffer. The type `T` must be layout compatible with the pointer’s `Element` type.
