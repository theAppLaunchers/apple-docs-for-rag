...
Local storage drivers receive their calls to `setPowerState` in a non-deterministic order from any other drivers. As a result, disk I/O and pageable memory may be unavailable in your implementation of `setPowerState`. Any attempt to access pageable memory, which may require I/O, can result in a stall, and eventually panic.
