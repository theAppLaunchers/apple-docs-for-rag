...
`Index of Refraction`  
The index of refraction the node uses if the surface is translucent or specular. The `Index of Refraction` defines how light bends or refracts when it passes through a material. The default value is `1.5`.

`Ambient Occlusion`  
The degree of ambient lighting that the surface receives. This value simulates soft shadows and subtle shading. A value of `0.0` indicates a fully occluded area, while a value of `1.0` indicates an unoccluded area. The default value is `1.0`.

`Opacity Threshold`  
The threshold for whether the node renders a portion of the surface based on its opacity level. A value of `0.0` means that no additional masking occurs. If the value is greater than `0.0`, the node renders only areas of the surface with an `Opacity` value greater than the value of this parameter. The default value is `0.0`.
