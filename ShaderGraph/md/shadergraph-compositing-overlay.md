...
The Overlay node has one of two effects. If `F+B` is less than `0.5`, then the node outputs a value of `2*F*B`. If `F+B` is greater than or equal to `0.5`, then it outputs`1-(1-F)(1-B)`, which creates the same visual effect as the Screen node. Visually the node makes dark areas of the blended texture even darker and light areas of the blended texture even lighter.
