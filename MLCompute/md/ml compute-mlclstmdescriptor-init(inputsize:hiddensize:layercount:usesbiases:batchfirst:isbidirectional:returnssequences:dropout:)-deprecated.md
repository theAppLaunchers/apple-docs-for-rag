...
- ML Compute
- MLCLSTMDescriptor
-  init(inputSize:hiddenSize:layerCount:usesBiases:batchFirst:isBidirectional:returnsSequences:dropout:) Deprecated
...
A Boolean that indicates whether the layer returns output for all sequences. If `false`, this method returns the output for only the last sequence. The default value is `true`.
