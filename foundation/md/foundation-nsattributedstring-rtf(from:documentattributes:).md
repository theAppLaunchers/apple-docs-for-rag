...
When writing data to the pasteboard, you can use the `NSData` object as the first argument to the NSPasteboard method setData(_:forType:), with a second argument of `NSRTFPboardType`. Although this method strips attachments, it leaves the attachment characters in the text itself. The `NSText` method rtf(from:), on the other hand, does strip attachment characters when extracting RTF.
