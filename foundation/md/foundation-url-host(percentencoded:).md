...
The system doesn’t allow certain characters in the URL host component, so URL percent-encodes those characters to create a valid URL. Calling this function with `percentEncoded = false` removes any percent-encoding and returns the unencoded host.

If the URL doesn’t contain a host component according to RFC 3986, this function returns `nil`.
