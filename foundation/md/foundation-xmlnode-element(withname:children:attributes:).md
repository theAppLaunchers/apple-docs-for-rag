...
An array of `NSXMLElement` objects or `NSXMLNode` objects of kinds XMLNode.Kind.element, XMLNode.Kind.processingInstruction, XMLNode.Kind.comment, and XMLNode.Kind.text. Specify `nil` if there are no children to add to this node object.

`attributes`  

An array of `NSXMLNode` objects of kind XMLNode.Kind.attribute. Specify `nil` if there are no attributes to add to this node object.
