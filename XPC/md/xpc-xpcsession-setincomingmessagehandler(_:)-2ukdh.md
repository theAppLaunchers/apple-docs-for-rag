...
``` source
func setIncomingMessageHandler(_ incomingMessageHandler: @escaping (Message) -> (any Encodable)?) where Message : Decodable
```
...
A closure that the session invokes when it receives messages. The closure has a parameter that contains the message from the client, and optionally returns an encodable reply message to returns to the client. If the closure returns Nil, you can use send(message:) to reply asynchronously after the closure completes.
