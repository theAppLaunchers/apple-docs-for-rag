...
``` source
func xpc_dictionary_set_uuid(
    _ xdict: xpc_object_t,
    _ key: UnsafePointer,
    _ uuid: UnsafePointer
)
```
...
The `uuid_t` value to insert. After calling this method, the XPC object corresponding to the primitive value inserted may be safely retrieved with xpc_dictionary_get_value(_:_:).
