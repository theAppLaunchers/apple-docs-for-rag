...
``` source
final func resolve(
    id: LocalTestingDistributedActorSystem.ActorID,
...
This function should either return an existing actor reference, or `nil` to signal that a remote distributed actor “proxy” should be created for this LocalTestingDistributedActorSystem.ActorID. If the resolve fails, meaning that it can neither locate a local actor managed by this actor system, nor identify that the identity is located on some remote actor system, then this function should throw.
